...
x=Final$Light_intensity
#x=16.6554481#-0.198197
#y = 8.440657 + 12.19154*x - 0.6148812*x^2 + 0.02206417*x^3 - 0.0005157655*x^4 + 0.000007797485*x^5 - 0.00000007528866*x^6 + 0.0000000004475405*x^7 - 0.000000000001492447*x^8 + 0.000000000000002138769*x^9
y = 8.440657 + 12.19154*x - 0.6148812*x^2 + 0.02206417*x^3 - 0.0005157655*x^4 + 7.797485e-06*x^5 - 7.528866e-08*x^6 + 4.475405e-10*x^7 - 1.492447e-12*x^8 + 2.138769e-15*x^9
...
#r<-polyroot(c(8.440657-99, 12.19154, -0.6148812, 0.02206417, -0.0005157655, 0.000007797485, 0.00000007528866, 0.0000000004475405, -0.000000000001492447, 0.000000000000002138769))
Real = vector(,p)
p<-seq(1,255,1)
Real = vector(,255)
for (i in p){
#r<-polyroot(c(8.440657-i, 12.19154, -0.6148812, 0.02206417, -0.0005157655, 0.000007797485, 0.00000007528866, 0.0000000004475405, -0.000000000001492447, 0.000000000000002138769))
r<- polyroot(c(8.440657-i, 12.19154, -0.6148812, 0.02206417, -0.0005157655, 7.797485e-06, -7.528866e-08, 4.475405e-10, -1.492447e-12, 2.138769e-15))
for (j in r){
  if(round(Im(j),7)==0)
    #print(Re(j))
    Real[i]<-Re(j)
...
x=Real[[i]]#-0.198197
y[[i]] = 8.440657 + 12.19154*x - 0.6148812*x^2 + 0.02206417*x^3 - 0.0005157655*x^4 + 7.797485e-06*x^5 - 7.528866e-08*x^6 + 4.475405e-10*x^7 - 1.492447e-12*x^8 + 2.138769e-15*x^9
#y[[i]] = 8.440657 + 12.19154*x - 0.6148812*x^2 + 0.02206417*x^3 - 0.0005157655*x^4 + 0.000007797485*x^5 - 0.00000007528866*x^6 + 0.0000000004475405*x^7 - 0.000000000001492447*x^8 + 0.000000000000002138769*x^9
}

y
Real

Ultimate<-data.frame(Real,y)
names(Ultimate)<-c( "Final_Light_intensity", "Verified_Predicted_y")
# Write CSV in R
